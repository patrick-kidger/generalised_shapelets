...
We now investigate the benefit of learning lengths. To do so, we begin by taking our usual model - linear on top of the generalised shapelet transform - but fix the lengths, rather than learning them differentiably. We then perform a hyperparameter search to determine the best and worst lengths for this model on each dataset.

We then train a model with differentiably learnt lengths initialised at the \emph{worst} lengths, and compare it to the \emph{best} performer from the hyperparameter search. We will see that the performance is comparable! This demonstrates lengths learnt differentiably perform just as effectively as those selected by hyperparameter search, but without the additional step of having to do so.

The classical shapelet transform does not apply to irregularly sampled or partially observed time series, prevent direct comparisons. So as to keep things interesting, and to provide benchmarks on such datasets, we use such datasets for our investigation into differentiably learning lengths. We consider the JapaneseVowels, Libras and LSST datasets from the UEA archive as before (selected as representing a range of difficulties), and drop either 10\%, 30\% or 50\% of the data for each channel of the dataset. The data dropped is independently selected for every channel of each time series, and is the same for every model and repeat.
